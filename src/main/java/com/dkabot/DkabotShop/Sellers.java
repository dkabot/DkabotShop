...
public class Sellers implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(cmd.getName().equalsIgnoreCase("sell")) {
			...
			instancesOfItem = plugin.all(player.getInventory(), material);
			...
			if(!plugin.contains(player.getInventory(), material, amount)) {
				sender.sendMessage(ChatColor.RED + "You must have the item you wish to sell");
				sender.sendMessage(ChatColor.RED + "in your inventory!");
				return true;
			}
			...
			if(!sender.hasPermission("dkabotshopadmin.bypassMaxStock")) {
				...
				if(maxStock != -1) {
					Boolean check = false;
					if(DBClass != null) check = (DBClass.getAmount() + amount > maxStock);
					else check = (amount > maxStock);
					if(check) {
			...
			if(!itemsNotRemoved.isEmpty()) {
				...
				//You must be doing something wrong~
					...
					if(amountRemoved > 0) player.getInventory().addItem(new ItemStack(material.getType(), amountRemoved, durability));
					sender.sendMessage(ChatColor.RED + "You must have the item you wish to sell");
					sender.sendMessage(ChatColor.RED + "in your inventory!");
					return true;
				}
			...
			//Item is in shop, modify the entry
				...
				if(cost == null) plugin.broadcastMessage(plugin.formatMessage("Added", sender.getName(), plugin.itemDB.rget(material.getTypeId(), material.getDurability()).toUpperCase(), amount, null, null));
				else {
